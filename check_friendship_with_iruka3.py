...
friendships = {}
friendship_file = 'itm_enterprise_tweet_09933828034109440_tweet_friendships.txt'
...
def check_friendship():
        ...
        no_rt = len(retweeters)


        for i in range(21,no_rt):
                flwrs = []
                for j in range(i,no_rt):
                        status = api.show_friendship(source_screen_name=retweeters[i],target_screen_name=retweeters[j])
                        if status[1].following:
                                flwrs.append(retweeters[j])
                friendships[retweeters[i]]=flwrs
                with open(friendship_file,"a") as input_file:
                        line = '{}, {}'.format(retweeters[i],flwrs)
